...
public class FileServiceImpl implements FileService{
    ...
    public boolean delete(Long no) {
        ...
        delete(file);               // 파일 삭제
        int result = fileMapper.delete(no);     // DB 데이터 삭제
        return result > 0;
    }
    ...
    public boolean upload(Files file) throws Exception {
        ...
        // 1. 파일 시스템에 등록 (파일 복사)
        // - 파일 정보: 원본파일면, 파일용량, 파일데이터, 파일명, 파일경로
    ...
    public int deleteByParent(Files file) {
        ...
        // 파일 삭제
        for (Files deleteFiles : fileList) {
            delete(deleteFiles);
        }
    ...
    public int deleteFilesList(List<Long> noList) {
        ...
        count = fileMapper.deleteFilesList(noList);
        log.info("파일 " + count + "개를 삭제하였습니다.");
        return count;
    }
    ...
    public boolean thumbnail(String id, HttpServletResponse response) throws Exception {
        ...
        // core.io -> import
        Resource resource = resourceLoader.getResource("classpath:statc/img/no-image.jpg");
        if (filePath == null || !(imgFile = new File(filePath)).exists() ) {
            // no-image.jpg (기본 이미지) 적용
            imgFile = resource.getFile();
            filePath = imgFile.getPath();
        }
        // 확장자
        // C:/upload/강아지.png
        // substring: 문자 잘라냄 -> .png 빼고 잘라냄
        String ext = filePath.substring(filePath.lastIndexOf(".") + 1);
        // png, jpeg, webp, gif 등 확장자 지정
        ...
        int result = FileCopyUtils.copy(fis, sos);              // 파일 전송(복사 개념)
        // copy = 0byte 아니라면 return
        return result > 0;
    }

...
/**
 * JWT 인증 필터
 * - /login 엔드포인트로 POST 요청 시, JSON body에서 username/password 를 읽어 인증 처리
...
public class JwtAuthenticationFilter extends UsernamePasswordAuthenticationFilter{
    ...
    public JwtAuthenticationFilter(AuthenticationManager authenticationManager, JwtProvider jwtProvider) {
        ...
        setFilterProcessesUrl( SecurityConstants.LOGIN_URL ); // 아래 주소 상수로 정의
        // setFilterProcessesUrl( "/login" );
    }

    // 인증 시도 메서드 3가지 핵심 포인트!!!
    // 인증 시도: request [ header | body { "username","password"}]
    @Override
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
    // attemptAuthentication : usernmae, password 꺼내서 인증 객체 만듦 -> 로그인 시도
    ...
    // 인증 성공: response [header {Authoraztion: "Bearer {jwt"} } body]

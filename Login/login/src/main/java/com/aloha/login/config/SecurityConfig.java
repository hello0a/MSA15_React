...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        ...
        // addFilterAt(...) : Spring Security의 username~ 자리에 내가 만든 로그인 필터(JwtAuthenticationFilter) 넣겠다 -> 내 방식으로 바꾸겠다 의미
        // addFilterBefore(...) : 일반 인증 필터 전에 JwtRequestFilter 먼저 돌려서 토큰 검사하고 인증 상태 세킹하겠다
        http.addFilterAt( new JwtAuthenticationFilter(authenticationManager, jwtProvider) , UsernamePasswordAuthenticationFilter.class ) // 로그인 인증 처리
            .addFilterBefore(new JwtRequestFilter(authenticationManager, jwtProvider), UsernamePasswordAuthenticationFilter.class);  // 유효한지 검증
        return http.build();
    }
    /**
   * CORS 설정 - 프론트엔드(localhost:5173) 허용
   * : 다른 출처(origin)의 서버에 요청을 허용할지 말지 결정하는 보안 정책
   */
  @Bean
  public CorsConfigurationSource corsConfigurationSource() {
    CorsConfiguration configuration = new CorsConfiguration();
    configuration.setAllowedOrigins(Arrays.asList(  // 허용된 URL 저장
      "http://localhost:5173",
      "http://127.0.0.1:5173"
    ));
